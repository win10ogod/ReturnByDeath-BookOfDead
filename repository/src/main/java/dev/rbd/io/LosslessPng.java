...
public final class LosslessPng {
    ...
    public static byte[] encode(int width,int height,int[] abgr) throws IOException {
        ...
        for(int part=0;part<parts;part++){int from=height*part/parts,to=height*(part+1)/parts;jobs.add(ENCODERS.submit(()->stripe(width,from,to,abgr)));}
    ...
    private static Stripe stripe(int width,int from,int to,int[] abgr) throws IOException {
        ...
        for(int y=from;y<to;y++){
                ...
                for(int x=0;x<width;x++){
                    ...
                    raw[offset+3]=(byte)((pixel>>>24)-(previous>>>24));previous=pixel;
                }
        }
