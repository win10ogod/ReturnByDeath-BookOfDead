...
public record MemoryFrame(long tick, String dimension, double x,double y,double z,float yaw,float pitch,
                          ...
                          String visualSource,int width,int height,int[] pixels,String png,SomaticState body,int sampleTicks) {
    /** Existing integrations and pre-0.7.3 frames retain their original one-tick cadence. */
    public MemoryFrame(long tick,String dimension,double x,double y,double z,float yaw,float pitch,
                       float health,String caption,List<Contact> contacts,List<Sound> sounds,String visualSource,
                       int width,int height,int[] pixels,String png,SomaticState body){
        this(tick,dimension,x,y,z,yaw,pitch,health,caption,contacts,sounds,visualSource,width,height,pixels,png,body,1);
    }
