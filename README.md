...
客戶端和伺服器都安裝同版本的 `rbd-0.6.1.jar`，並移除舊版。首次進入單人世界會自動授予死亡回歸並建立初始回歸點；聊天訊息提供書庫座標。Esc 退出閱讀，F8 降低感官刺激。
...
建立世界的「遊戲規則」畫面可設定 **97 項 RBD 規則**。既有世界可從「編輯」→「遊戲規則」修改；多人服也能使用原生 `gamerule` 指令：
...
0.6.1 以兩名實際客戶端通過 **41 項整合檢查**，涵蓋騎士武器發射、飾品原生效果、暮色首領處決、跨維度追擊、學習記錄與光環觸發回歸後維持連線。0.6.1 的 **18 項實機 AI 檢查**已驗證原生射擊、退距、箭矢閃避、繞牆，以及實際鍵盤移動和持盾的應對。NeoForge GameTests 共 **16 項**，包含裝備元件複製、二階段、處決與範圍豁免、原生弓弩彈藥、移動預判、學習及規則保存。
